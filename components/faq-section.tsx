...
const faqs = [
  {
    ...
    answer:
      "Nos encontramos en Cota Cundinamarca, a pocos minutos de la ciudad pero rodeados de naturaleza. Con gusto le compartimos la ubicación exacta y coordinamos una visita cuando lo desee.",
  },
  {
...
export function FaqSection() {
  ...
  return (
    <section className="bg-[#F5F1E8] py-28 md:py-40">
      <div
        id="preguntas"
        className="mx-auto max-w-4xl scroll-mt-40 px-6 md:px-10"
      >
        {/* Header */}
        <div className="mb-16 md:mb-24">
          <motion.span
            className="block text-[0.7rem] font-light uppercase tracking-[0.45em] text-[#556B2F]"
          ...
          <motion.h2
            className="mt-6 text-balance font-serif text-4xl font-light leading-[1.1] tracking-tight text-[#2D2D25] md:text-6xl"
        ...
        {/* FAQ */}
        <div className="border-t border-[#DDD4C4]">
          {faqs.map((faq, i) => {
            const isOpen = open === i
            ...
            return (
              <motion.div
                key={faq.question}
                className="border-b border-[#DDD4C4]"
                initial={{ opacity: 0, y: 16 }}
                whileInView={{ opacity: 1, y: 0 }}
              ...
              >
                <h3>
                  ...
                  >
                    <span className="font-serif text-xl font-light tracking-tight text-[#2D2D25] md:text-2xl">
                      {faq.question}
                    </span>

                    <span
                      className="mt-1 flex h-6 w-6 shrink-0 items-center justify-center text-[#556B2F]"
                      aria-hidden="true"
                    >
                      <motion.span
                ...
                <AnimatePresence initial={false}>
                  {isOpen && (
                    ...
                    >
                      <p className="max-w-2xl pb-8 pr-10 text-base font-light leading-relaxed text-[#6B6B5E] text-pretty">
                        {faq.answer}
                      </p>
                    </motion.div>
                  )}
